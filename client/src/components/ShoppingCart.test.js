...
test('displays plants in cart', () => {
    //renders cart and checks it rendered correctly
    render(<ShoppingCart cart={plants}/>);
    const rosso = screen.getByText(/peperomia rosso/i);
    expect(rosso).toBeInTheDocument();

    const stringD = screen.getByText(/string of dolphins/i);
    expect(stringD).toBeInTheDocument();
